...
class OrderManager {
  ...
  sendState (sendFn) {
    sendFn(JSON.stringify(['orderState', {asks: this.asks, bids: this.bids, currentAsks: this.currentAsks, currentBids: this.currentBids]));
  }
...
//    console.log('bids', this.bids);
  ...
  getAsksAtPrice (price) {
    if (this.asks.length === 0) return 0;

    const index = binarySearchFirstElementAscending (this.asks, price, 0, this.asks.length - 1);

    const c = this.asks[index];

    if (c) {
      if (c[0] === price) return c[1];
    }

    return 0;
  }

  getBidsAtPrice (price) {
    if (this.bids.length === 0) return 0;

    const index = binarySearchFirstElementDescending (this.bids, price, 0, this.bids.length - 1);
  ...
  setOrderQueue () {
    this.asksQueue.length = 0;
    this.bidsQueue.length = 0;

    const numberOfCurrentAsks = Object.keys(this.currentAsks);
    const numberOfCurrentBids = Object.keys(this.currentBids);

    for (var i = 0; numberOfCurrentAsks + i < this.maxAsks; i++) {
      
    }

    for (var i = 0; numberOfCurrentBids + i < this.maxBids; i++) {

    }
  }

  orderComplete (id) {
    if (this.currentAsks[id]) {
      delete this.currentAsks[id];
    }
    else if (this.currentBids[id]) {
      delete this.currentBids[id];
    }
  }

  orderCanceled (id) {
    this.orderComplete(id);
  }
}
         ...
         .then(result => {
           for (let i = 0; i < os.length; i++) {
             if (o === os[i]) {
               ...
               if (side === 'asks' && managers[ticker].getBidsAtPrice(price) > 0) {
                 managers[ticker].removeBid(price);
               }
               else if (managers[ticker].getAsksAtPrice(price) > 0) {
                 managers[ticker].removeAsk(price);
               }
               else {
                 if (side === 'asks') {
                   managers[ticker].addAsk(price);
                 }
                 else {
                   managers[ticker].addBid(price);
                 }
               }
